...
const updateUser = async (req, res) => {
  try {
    const { fullName, username, email, password } = updateUserSchema.parse(
      req.body
    );
    ...
    if (username && username === userExists.username) {
      return res
        .status(400)
        .json({ message: "Username is the same as the old one" });
    }

    if (email && email === userExists.email) {
      return res
        .status(400)
        .json({ message: "Email is the same as the old one" });
    }
    ...
    if (password) {
      ...
      if (await bcrypt.compare(password, userExists.password)) {
        return res
          .status(400)
          .json({ message: "Password is the same as the last one entered" });
      }
    }
